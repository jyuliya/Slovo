...
Вид приложения - три экрана: стартовый, поисковый и с каждым словом. Переход от одного экрана к другому осуществляется с помощью делегирования, так же при переходе от поискового экрана передается значение, обозначающее выбираемое или искомое слово. При нажатии на поиск выскакивает клавиатура в нижней части экрана.
...
На стартовом экране появляется название программного продукта и кнопки "Начать поиск" и "Выход".

При нажатии кнопки "Начать поиск" появляется экран с поисковой строкой, в которую можно вводить слова и таблица со всеми словами в словаре. Внизу экрана расположена кнопка "Назад" для возврата к стартовому экрану.

При выборе слова из таблицы или поиска слова в поисковой строке появляется новый экран со словом и его определением и кнопкой "Назад", которая возвращает на предыдущий экран.

### VI. Заключение

В ходе работы самое главное, что я поняла - даже самы простой атрибут, который на прототипе очень легко выглядит, иногда вызывает большие затруднения, связанные с особенностями языка программирования. Но если внимательнее разобраться в нужной теме, обязательнонайдется идеальное решение.
Все знания, полученные за время выполнения - очень полезные и прикладные, востребованные во многих областях программирования, хотя дизайн остается слабым местом для любого программиста, так как для иделаьного дизайна приложения нужно соблюдать определенные цвета и пропорции, более привлекающие взгляд пользователя. 

Само приложение очень полезное для любого человека в нашевремя, даже используя интернет-сленг я сама нашла много незнакомых мне слов. 
